""" GLS code to render multi-colored glyphs defined in the COLR Opentype table:
    https://docs.microsoft.com/en-us/typography/opentype/spec/colr

GLSL shader code based on the following public sources:\
    :download:`[source] <../../../litGL/glsl_color.py>`
...
uniform usampler2DRect u_colorsTex;
...
void main()
{
    ...
    uvec2 colorLoc = uvec2(fs_in.glyphParam.xy);

    // Rendered glyph pixel size
    vec2 pixelsPerEm = vec2(1.0 / emsPerPixel);

    uvec2 layerInfo = texelFetch(u_colorsTex, ivec2(colorLoc)).xy;

   for(uint layer = 0U; layer < layerInfo.x; layer++)
    {
        xcov = 0.0;
        coverage = 0.0;
        // Colored glyph layer index in the color texture
        uvec4 layerData = texelFetch(u_colorsTex, ivec2(layerInfo.y + layer, 0));

        // Glyph index in the bands texture (x, y)
        uvec2 glyphLoc = uvec2(layerData.xy);

        int nBands = int(layerData.z);
        ivec2 bandIndex = ivec2(clamp(ivec2(fs_in.texCoord * nBands), ivec2(0, 0), ivec2(nBands-1, nBands-1)));
        ...
        for (uint curve = 0U; curve < hBandData.x; curve++)
        {
            ...
            ivec2 curveLoc = ivec2(texelFetch(u_bandsTex, ivec2(hbandLoc.x + curve, hbandLoc.y)).xy);
        ...
        // Recover band data for the current pixel reading the texture header, number of curves in
        // the band (-x-)and offset to indices of curves in the curves texture (-y-)
        uvec2 vBandData = uvec2(texelFetch(u_bandsTex, ivec2(int(glyphLoc.x) + nBands + bandIndex.x, int(glyphLoc.y))).xy);
        // Curve indices band offset
        ...
        {
            // Index of curve in the curve texture
            ivec2 curveLoc = ivec2(texelFetch(u_bandsTex, ivec2(vbandLoc.x + curve, vbandLoc.y)).xy);
            // Retrieve p1 and p2 control points of the current curve
            vec4 p12 = texelFetch(u_curvesTex, curveLoc) - vec4(fs_in.texCoord, fs_in.texCoord);
            // Retrieve p3 control point of the current curve
            vec2 p3 = texelFetch(u_curvesTex, ivec2(curveLoc.x+1, curveLoc.y)).xy - fs_in.texCoord;
        ...
        // Calculate glyph coverage
        float a = abs(xcov+ycov);
        float b =  (1.0-(abs(xcov))+(1.0-abs(ycov)));
        coverage = sqrt(clamp(sqrt((a)/(a+b)), 0.0, 1.0));

        // Get position of the glyph color
        uvec2 layerColorPos;
        layerColorPos.x = uint(layerData.w);
        layerColorPos.y = 0U;

        // Extract from the color textrure the layer color
        vec4 color = texelFetch(u_colorsTex, ivec2(layerColorPos));
        // Reduce in the range 0.-1.
        color = color /255.0;
        tempColor = vec4(mix(tempColor.xyz, color.xyz, coverage), clamp(tempColor.w + coverage, 0, 1.0));
    }

     float alpha = tempColor.w * u_color.w;
